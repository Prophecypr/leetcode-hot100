...
> 不定长滑动窗口 · 求最长/最大（基础）· 双计数器同时约束

**与前面题目的区别：**
| 题号 | 收缩条件 | 数据结构 |
|------|----------|----------|
| 3/3090/2958 | `cnt[x] > k`（单种超标） | unordered_map |
| 1493 | `cnt0 > 1`（单种超标） | int |
| **2024** | **`cnt[0] > k && cnt[1] > k`** | **int cnt[2]** |

**为什么是 `&&` 不是 `||`：**
- 窗口合法条件：翻转最多 k 个字符后能全同
- 等价于：`cntT ≤ k`（全翻成 F）**或** `cntF ≤ k`（全翻成 T）
- 反过来：不合法当且仅当 **`cntT > k` 且 `cntF > k`**（两种都翻不起）
- 所以收缩条件是 `&&`，不是 `||`
...
> 不定长滑动窗口 · 求最长/最大（基础）· 收官题 ✅

**与 1493 的关系：**
| 题号 | 约束 | 与 k 的关系 |
|------|------|------------|
| 1493 | 最多 1 个 0 | **k=1 的硬编码版** |
| **1004** | **最多 k 个 0** | **参数化版本** |

实际上 1004 才是通解，1493 只是它的特例（k=1）。

**§2.1 不定长·求最长/最大（基础）10/10 全部完成 🎉**

| 题号 | 约束 | 技巧 |
|------|------|------|
| 3 | 无重复 | unordered_map |
| 3090 | 最多 2 次 | 改 >1→>2 |
| 1493 | 最多 1 个 0 | `right-left`, k=1 特例 |
| 3634 | 比值 ≤ k | 排序 + `1LL` |
| 1208 | 成本 ≤ 预算 | 一维约束直板 |
| 904 | 最多 2 种水果 | `cnt.size()>2` + `erase` |
| 1695 | 无重复 | `set` + 求最大和 |
| 2958 | 每种 ≤k | 参数化通解 |
| 2024 | 双计数器 | `cntT>k && cntF>k` |
| **1004** | **最多 k 个 0** | **收官题** |
...
for (int right = 0; right < n; right++) {
    cnt[nums[right]]++;
    while (cnt[0] > k) {  // 1493 把 k 换成 1 就是同一道题
        cnt[nums[left]]--;
        left++;
    }
    ans = max(ans, right - left + 1);
...
| 题号 | 条件 | 场景 |
|------|------|------|
| 3634 | `nums[l] * k < nums[r]` | 元素比值限制 |
| **2779** | **`nums[r] - nums[l] > 2k`** | **元素差值限制** |

**思路：**
- 每个元素可增/减 k → 若要使其相等，跨度需 ≤ 2k
- 排序后滑动窗口找最长子数组，条件 `nums[r] - nums[l] ≤ 2k`
- 排序 + 滑动窗口在"任意选取子序列"问题中是经典套路
